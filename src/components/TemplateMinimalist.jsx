...
function TemplateMinimalist() {
  ...
  const scrollOpen = () => { myRefOpen.current.scrollIntoView({top: 0, left: 0, behavior: 'smooth' }) }   
  const scrollBack = () => { myRefBack.current.scrollIntoView({top: 0, left: 0, behavior: 'smooth' }) }   
			...
			<div ref={myRefOpen} className="cobascroll">
				<div className="cm-title-studioname-info">
					...
					<div className="cm-info">
						<div>
							<FaRegShareSquare style={{marginBottom:"3px"}}/> <span className="share">Share</span>
						</div>
						<div>
							<BsBoxArrowInDown style={{marginBottom:"3px"}}/> <span>Download</span>
						</div>
					</div>
				</div>
